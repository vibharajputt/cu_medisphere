...
export default function SignupPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    phone: '', role: 'PATIENT', address: '', city: '', referralCode: '',
  ...
  const handleNext = () => {
    if (step === 1) {
      ...
      if (formData.role === 'PATIENT' || formData.role === 'FACULTY') {
        if (!formData.city) {
        ...
        if (formData.role === 'PATIENT') {
          if (!formData.collegeUid) {
            toast.error('Please enter your college UID.');
            return;
          }
          const emailDomain = formData.email.split('@')[1]?.toLowerCase();
          if (emailDomain !== 'cuchd.in' && emailDomain !== 'cumail.in') {
            toast.error('Registration is restricted to Chandigarh University email addresses only (@cuchd.in or @cumail.in).');
            return;
          }
          const uidRegex = /^\d{2}[A-Za-z]{2,5}\d{3,6}$/;
          if (!uidRegex.test(formData.collegeUid)) {
            toast.error('Invalid Chandigarh University UID format. Please use a valid format (e.g., 24BCF10013).');
            return;
          }
        }

        if (formData.role === 'FACULTY') {
          if (!formData.collegeEid) {
            toast.error('Please enter your college EID.');
            return;
          }
          const emailDomain = formData.email.split('@')[1]?.toLowerCase();
          if (emailDomain !== 'cuchd.in' && emailDomain !== 'cumail.in') {
            toast.error('Faculty registration is restricted to Chandigarh University email addresses only (@cuchd.in or @cumail.in).');
            return;
          }
        }

        if (formData.isHosteller) {
          ...
          if (!formData.hostelWarden) {
            toast.error('Please enter your hostel warden name.');
            return;
          }
          if (!formData.wardenEmail) {
            toast.error('Please enter your warden email.');
            return;
          }
          if (!formData.wardenNumber) {
            toast.error('Please enter your warden phone number.');
            return;
          }
          if (formData.wardenNumber.length !== 10) {
            toast.error('Warden phone number must be exactly 10 digits.');
            return;
          }
        }
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const submitRole = formData.role === 'FACULTY' ? 'PATIENT' : formData.role;
          ...
          <AnimatePresence mode="wait">
            ...
            {step === 1 && (
              ...
              >
                ...
                <div className="form-group">
                  ...
                  <div className="form-input-icon">
                    ...
                    <input
                      type="email"
                      className="form-input"
                      placeholder={formData.role === 'PATIENT' ? "Enter your student email (e.g., student@cuchd.in)" : "Enter your email address"}
                ...
                {(formData.role === 'PATIENT' || formData.role === 'FACULTY') && (
                  <>
                    <div className="form-group">
                    ...
                    {/* Student College UID field */}
                    {formData.role === 'PATIENT' && (
                      <div className="form-group" style={{ marginTop: '16px' }}>
                        <label className="form-label">College UID *</label>
                        <input
                          type="text"
                          className="form-input"
                          placeholder="Enter your College UID"
                          value={formData.collegeUid || ''}
                          onChange={(e) => updateField('collegeUid', e.target.value)}
                          required
                        />
                      </div>
                    )}

                    {/* Faculty College EID field */}
                    {formData.role === 'FACULTY' && (
                      <div className="form-group" style={{ marginTop: '16px' }}>
                        <label className="form-label">College EID *</label>
                        <input
                          type="text"
                          className="form-input"
                          placeholder="Enter your College EID"
                          value={formData.collegeEid || ''}
                          onChange={(e) => updateField('collegeEid', e.target.value)}
                          required
                        />
                      </div>
                    )}

                    {/* Referral Code / Link Field for Student & Faculty */}
                    {(formData.role === 'PATIENT' || formData.role === 'FACULTY') && (
                      <div className="form-group" style={{ marginTop: '16px' }}>
                        <label className="form-label">
                          Referral Code / Referral Link <span style={{ opacity: 0.6, fontSize: '0.8rem', fontWeight: '400' }}>(Optional)</span>
                        </label>
                        <div className="form-input-icon">
                          <FiGift className="icon" style={{ color: '#00b4b6' }} />
                          <input
                            type="text"
                            className="form-input"
                            placeholder="Enter Referral Code (e.g. REF-24BCF10013) or paste Referral Link"
                        ...
                        <small style={{ color: '#0d9488', fontSize: '0.74rem', marginTop: '6px', display: 'block', fontWeight: '500' }}>
                          🎁 Enter a referral code or paste referral link to earn bonus health points on first login!
                        </small>
                      </div>
                    )}

                    {/* Hosteller toggle for both Student and Faculty */}
                    <div className="form-group" style={{ marginTop: '16px' }}>
                      <label className="checkbox-pill" style={{ display: 'inline-flex', padding: '8px 16px', background: 'rgba(255,255,255,0.02)', border: '1px solid var(--border-color)', borderRadius: '24px', cursor: 'pointer', gap: '8px', width: '100%' }}>
                        <input
                          type="checkbox"
                          checked={formData.isHosteller || false}
                          onChange={(e) => {
                            updateField('isHosteller', e.target.checked);
                            if (!e.target.checked) {
                              setFormData(prev => ({
                                ...prev,
                                hostelName: '',
                    ...
                    {/* Hostel Details conditional view */}
                    {formData.isHosteller && (
                      <>
                          ...
                          <div className="form-group">
                            ...
                            >
                              ...
                              {formData.gender === 'Female' && (
                                <>
                                  <option value="Sukhna Hostel">Sukhna Hostel</option>
                                  <option value="LC hostel">LC hostel</option>
                                  <option value="Shivalik hostel">Shivalik hostel</option>
                                  <option value="Govind hostel">Govind hostel</option>
                                  <option value="Tagore girls hostel">Tagore girls hostel</option>
                                </>
                              )}
                              ...
                              {formData.gender !== 'Male' && formData.gender !== 'Female' && (
                                <option disabled value="">Please select Male or Female gender first</option>
                              )}
                            </select>
                          </div>
                        ...
                        <div className="grid grid-2">
                          ...
                          <div className="form-group">
                            <label className="form-label">Hostel Warden *</label>
                            <input
                              type="text"
                              className="form-input"
                              placeholder="Warden Name"
                              value={formData.hostelWarden || ''}
                              onChange={(e) => updateField('hostelWarden', e.target.value)}
                              required
                            />
                          </div>
                        </div>
                        ...
                        <div className="grid grid-2">
                          <div className="form-group">
                            <label className="form-label">Warden Email *</label>
                            <input
                              type="email"
                              className="form-input"
                              placeholder="warden@example.com"
                              value={formData.wardenEmail || ''}
                              onChange={(e) => updateField('wardenEmail', e.target.value)}
                              required
                            />
                          </div>

                          <div className="form-group">
                            <label className="form-label">Warden Phone Number *</label>
                            <div className="form-input-icon">
                              <FiPhone className="icon" />
                              <input
                                type="tel"
                                ...
                                onChange={(e) => {
                                  ...
                                  updateField('wardenNumber', cleaned);
                                }}
                                maxLength={10}
                                required
                              />
                      ...
                      <div className="form-group" style={{ padding: '16px', border: '1px dashed var(--primary)', borderRadius: '12px', background: 'rgba(0, 217, 166, 0.02)', marginTop: '16px' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginTop: '8px' }}>
                          ...
                          {formData.prescriptionReportUrl && (
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '0.85rem', color: '#00b4b6', marginTop: '4px' }}>
                          ...
                          onChange={async (e) => {
                            ...
                            try {
                              ...
                              try {
                                const res = await fileAPI.upload(file);
                                if (res.data.success) {
                                  updateField('images', [res.data.message || base64Url]);
                                } else {
                                  updateField('images', [base64Url]);
                                }
                              } catch (apiErr) {
                                updateField('images', [base64Url]);
                              }
                    ...
                    <div className="form-group">
                      ...
                      <div className="form-input-icon">
                        ...
                        {!licenseVerifying && licenseVerified && formData.licenseNo && (
                          <FiCheck className="icon" style={{ color: '#00b4b6', position: 'absolute', right: '12px' }} />
                        )}
                      </div>
                      ...
                      {licenseVerified && formData.licenseNo && (
                        <p style={{ color: '#00b4b6', fontSize: '0.8rem', marginTop: '4px' }}>License verified ✓</p>
                      )}
                    </div>
                    <div className="form-group">
                  ...
                  <>
                    <div className="form-group">
                      ...
                      {pharmacyLicenseVerified && formData.licenseNo && (
                        <p style={{ color: '#00b4b6', fontSize: '0.8rem', marginTop: '4px' }}>Pharmacy license verified ✓</p>
                      )}
                    </div>
                        ...
                        onChange={async (e) => {
                          ...
                          try {
                            ...
                            try {
                              const res = await fileAPI.upload(file);
                              if (res.data.success) {
                                updateField('profilePhoto', res.data.message || base64Url);
                              } else {
                                updateField('profilePhoto', base64Url);
                              }
                            } catch (apiErr) {
                              updateField('profilePhoto', base64Url);
                            }
                  ...
                  <label style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', cursor: 'pointer', fontSize: '0.9rem', color: 'var(--text-primary)' }}>
                    ...
                    <span>
                      ...
                      <span
                        ...
                        style={{ color: '#042a59', textDecoration: 'underline', fontWeight: 600, cursor: 'pointer' }}
                      >
          ...
          <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
            ...
            >
              <h3 style={{ fontSize: '1.4rem', fontWeight: 700, marginBottom: '16px', borderBottom: '1px solid rgba(255,255,255,0.08)', paddingBottom: '12px', color: 'var(--primary)' }}>
                Terms & Conditions for {formData.role ? (formData.role === 'PATIENT' ? 'Students' : (formData.role === 'FACULTY' ? 'Faculty Members' : formData.role.charAt(0) + formData.role.slice(1).toLowerCase() + 's')) : 'Users'}
              </h3>
