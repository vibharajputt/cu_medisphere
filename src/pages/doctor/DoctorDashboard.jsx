...
export default function DoctorDashboard() {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      console.warn('Dashboard data load failed, using empty defaults:', error);
      setHospitals([]);
      setAppointments([]);
      setPrescriptions([]);
    } finally {
    ...
    <div className="cuims-layout animate-fade-in">
      ...
      <header className="cuims-header">
        ...
        <div className="cuims-header-right" style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          <button type="button" className="cuims-icon-btn" onClick={() => toast.info('Doctor settings panel active')}>
            <FiSettings />
          </button>
          ...
          <div className="cuims-user-chip" style={{ display: 'flex', alignItems: 'center', gap: '10px', paddingLeft: '8px', borderLeft: '1px solid #e2e8f0', cursor: 'pointer' }}>
            <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: '#0f766e', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold', fontSize: '0.9rem' }}>
              D
            </div>
            <div className="cuims-user-info" style={{ textAlign: 'right', lineHeight: '1.2' }}>
              ...
              <div className="cuims-user-id" style={{ fontSize: '0.68rem', color: '#64748b', fontWeight: '600' }}>DOCTOR</div>
            </div>
          </div>
      ...
      <div className="cuims-body-wrapper" style={{ display: 'flex', flex: 1 }}>
        ...
        <aside className={`cuims-sidebar ${sidebarCollapsed ? 'collapsed' : ''}`} style={{ width: sidebarCollapsed ? '60px' : '260px', background: '#303e67', color: '#ffffff', display: 'flex', flexDirection: 'column', transition: 'width 0.3s ease', flexShrink: 0, minHeight: 'calc(100vh - 64px)' }}>
          <div style={{ padding: '16px 20px', borderBottom: '1px solid rgba(255,255,255,0.08)', fontWeight: 800, fontSize: '0.92rem', letterSpacing: '0.5px' }}>
            {!sidebarCollapsed ? 'Doctor Portal' : '🩺'}
          </div>
          
          <ul className="cuims-sidebar-nav" style={{ listStyle: 'none', padding: '12px 0', margin: 0 }}>
            {[
              { id: 'hospitals', label: 'My Hospitals', icon: <FiActivity color="#00d9a6" />, action: () => setActiveTab('hospitals') },
              { id: 'appointments', label: 'Appointment Queue', icon: <FiUsers color="#042a59" />, badge: (appointments || []).filter(appt => appt.status === 'PENDING' || appt.status === 'CONFIRMED').length, badgeColor: '#00b4b6', action: () => setActiveTab('appointments') },
              { id: 'history', label: 'Appointment History', icon: <FiClock color="#94a3b8" />, badge: (appointments || []).filter(appt => appt.status === 'COMPLETED' || appt.status === 'CANCELLED').length, badgeColor: '#64748b', action: () => setActiveTab('history') },
              { id: 'ai-analyzer', label: 'AI Safety & Interaction A...', icon: <FiCpu color="#33c3c5" />, action: () => setActiveTab('ai-analyzer') },
              { id: 'prescribe', label: 'Write Prescription', icon: <FiFileText color="#ec4899" />, action: () => setActiveTab('prescribe') },
              { id: 'schedule', label: 'Schedule', icon: <FiCalendar color="#06b6d4" />, action: () => setActiveTab('appointments') },
              { id: 'alerts', label: 'Important Alerts', icon: <FiBell color="#1d467c" />, badge: 2, badgeColor: '#ef4444', action: () => {
                setActiveTab('history');
                toast.info('High-risk patient interaction alerts are active.');
              } }
            ].map(item => (
              <li 
                key={item.id}
                className={`cuims-sidebar-item ${activeTab === item.id || (item.id === 'schedule' && activeTab === 'appointments') ? 'active' : ''}`}
                onClick={() => {
        ...
        <main className="cuims-main-content">
          ...
          {/* Doctor Header Card */}
          <div className="glass-card doctor-header-card" style={{ display: 'flex', flexWrap: 'wrap', gap: '32px', padding: '32px', marginBottom: '40px', alignItems: 'center', background: '#ffffff', borderRadius: '16px', border: '1px solid #cbd5e1' }}>
            ...
            <div style={{ flex: 1, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '16px' }}>
              <div>
                <h1 className="heading-lg" style={{ marginBottom: '8px', fontSize: '1.8rem', fontWeight: 800 }}>
                  Welcome, <span className="text-gradient" style={{ background: 'linear-gradient(135deg, #0f766e, #7c3aed)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>{user.name}</span>
                </h1>
                <p style={{ color: '#64748b', marginBottom: '8px', fontWeight: 500 }}>{user.specialization || 'General Medical Practice'}</p>
                <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap', fontSize: '0.9rem' }}>
                  <span style={{ background: '#f1f5f9', padding: '4px 12px', borderRadius: '20px', fontWeight: 600, color: '#334155' }}>
