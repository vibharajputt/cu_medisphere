...
export default function StudentHealthPortalTab({ profileData, user }) {
  ...
  const handleApplyCoupon = (overrideCode) => {
    ...
    if (targetCode === 'CU20') {
      ...
      setAppliedCoupon({ code: 'MEDASTRA10', percent: 10, label: '⚡ 10% Extra MedAstraQ Discount' });
  ...
  const handleDownloadInvoice = (txn) => {
    try {
      ...
      doc.setFontSize(18);
      doc.text('MedAstraQ Campus Healthcare System', 14, 20);
      ...
      doc.text('This receipt is digitally generated by MedAstraQ CU Healthcare Portal.', 14, 170);

      doc.save(`MedAstraQ_Receipt_${txn.id}.pdf`);
