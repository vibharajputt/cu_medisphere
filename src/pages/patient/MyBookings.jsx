...
export default function MyBookings() {
  ...
  const handleOpenRatingModal = (booking) => {
    ...
    setSelectedRating(booking.userRating || 5);
    setHoverRating(0);
  ...
  const handleSubmitRating = async (e) => {
    ...
    try {
      ...
      await bookingAPI.rateBooking(ratingModalBooking.id, selectedRating, reviewText);
      toast.success(`Thank you! Rating & review submitted for ${ratingModalBooking.doctorName || 'Doctor'} ⭐`, { duration: 4000 });
  ...
  const fetchBookings = async () => {
    setLoading(true);
    const localBookings = JSON.parse(localStorage.getItem('medastrax_custom_bookings') || '[]') || [];
    const normalizedLocalBookings = localBookings.map(normalizeLocalBooking);

    try {
      const res = await bookingAPI.getPatientBookings(activeProfile ? activeProfile.id : null);
      const apiBookings = Array.isArray(res.data) ? res.data : [];
      const mergedBookings = [
        ...apiBookings,
        ...normalizedLocalBookings.filter((local) => !apiBookings.some((api) => String(api.id) === String(local.id)))
      ];
      setBookings(mergedBookings);
    } catch (error) {
      console.warn('Failed to load appointments from API, falling back to local bookings', error);
      setBookings(normalizedLocalBookings);
      if (activeProfile) {
        toast.error('Failed to load appointments from the server. Showing saved local appointments.');
      }
    } finally {
      setLoading(false);
    }
          ...
          <div className="bookings-grid">
            <AnimatePresence>
              ...
              >
                ...
                {(booking.status === 'COMPLETED' || !!booking.aiReport || filter === 'completed') && (
                  <div style={{
                    marginTop: '8px',
                    padding: '12px 14px',
                    background: 'rgba(255, 255, 255, 0.02)',
                    ...
                    {booking.userRating ? (
                      <div>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                          ...
                          <button 
                            ...
                            style={{ background: 'none', border: 'none', color: 'var(--primary)', cursor: 'pointer', fontSize: '0.78rem', display: 'flex', alignItems: 'center', gap: '4px', fontWeight: 600 }}
                          >
                            <FiEdit size={13} /> Edit
                          </button>
                        </div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '4px', margin: '4px 0' }}>
                          {[1, 2, 3, 4, 5].map((star) => (
                            <FiStar 
                              key={star} 
                              size={15} 
                              fill={star <= booking.userRating ? '#1d467c' : 'none'} 
                              color={star <= booking.userRating ? '#1d467c' : 'var(--text-muted)'} 
                            />
                          ))}
                          <span style={{ fontSize: '0.85rem', fontWeight: 700, color: '#1d467c', marginLeft: '6px' }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '8px' }}>
                        <div style={{ fontSize: '0.82rem', color: 'var(--text-secondary)', fontWeight: 600 }}>
                          Rate consultation with {booking.doctorName || 'Doctor'}
                        </div>
                        <button
                          type="button"
                          onClick={() => handleOpenRatingModal(booking)}
                          ...
                          style={{
                            ...
                            fontSize: '0.8rem',
                            borderRadius: '20px',
                            background: 'rgba(29, 70, 124, 0.12)',
                            color: '#1d467c',
                            border: '1px solid rgba(29, 70, 124, 0.3)',
                            fontWeight: 700,
                            display: 'flex',
                            alignItems: 'center',
                            gap: '6px',
                        ...
                        >
                          <FiStar fill="#1d467c" size={14} /> Rate & Review
                        </button>
                      </div>
                    )}
                  </div>
      ...
      {ratingModalBooking && (
        <div className="modal-backdrop" style={{
          ...
          <div className="glass-card animate-scale-up" style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <h3 className="heading-sm" style={{ margin: 0, fontSize: '1.2rem', display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <FiStar color="#1d467c" fill="#1d467c" /> Rate Consultation
                </h3>
                <span style={{ fontSize: '0.82rem', color: 'var(--text-secondary)' }}>
                  Share your feedback for {ratingModalBooking.doctorName || 'Practitioner'}
                </span>
              </div>
              <button 
            ...
            <div style={{
              ...
              <div style={{
                ...
                height: '44px',
                borderRadius: '50%',
                background: 'linear-gradient(135deg, #00b4b6, #06b6d4)',
                display: 'flex',
                alignItems: 'center',
                justifyContent: 'center',
                fontWeight: 800,
                ...
                {ratingModalBooking.doctorName?.charAt(0) || 'D'}
              </div>
              <div>
                <div style={{ fontWeight: 700, fontSize: '0.95rem', color: 'var(--text-primary)' }}>
                  {ratingModalBooking.doctorName || 'Consulting Practitioner'}
                </div>
                <div style={{ fontSize: '0.78rem', color: 'var(--text-secondary)' }}>
                  {ratingModalBooking.hospitalName} • {ratingModalBooking.type === 'ONLINE' ? 'Online Video Consult' : 'In-Person Visit'}
                </div>
              </div>
            </div>
                ...
                {[1, 2, 3, 4, 5].map((star) => {
                  ...
                  return (
                    ...
                    >
                      <FiStar 
                        size={32} 
                        fill={isFilled ? '#1d467c' : 'none'} 
                        color={isFilled ? '#1d467c' : 'var(--text-muted)'} 
                      />
                    </button>
                  );
                })}
              </div>
              <div style={{ fontSize: '0.86rem', fontWeight: 700, color: '#1d467c' }}>
                {selectedRating === 1 && '⭐ 1/5 - Poor Experience'}
                {selectedRating === 2 && '⭐⭐ 2/5 - Below Expectations'}
                {selectedRating === 3 && '⭐⭐⭐ 3/5 - Good / Average'}
                {selectedRating === 4 && '⭐⭐⭐⭐ 4/5 - Very Good'}
                {selectedRating === 5 && '⭐⭐⭐⭐⭐ 5/5 - Exceptional Care & Guidance!'}
              </div>
            </div>
            ...
            <div className="form-group">
              <label className="form-label" style={{ fontSize: '0.85rem', display: 'flex', justifyContent: 'space-between' }}>
                <span>Written Review / Feedback</span>
                <span style={{ color: 'var(--text-muted)', fontSize: '0.75rem' }}>(Optional)</span>
              </label>
              <textarea
                rows="3"
                className="form-input"
                placeholder="Share details about your experience, doctor's diagnosis, advice, or suggestions..."
                value={reviewText}
                onChange={(e) => setReviewText(e.target.value)}
                style={{ width: '100%', resize: 'none', padding: '12px', fontSize: '0.88rem' }}
              />
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px' }}>
              ...
              <button
                ...
                className="btn btn-primary"
                style={{
                  background: 'linear-gradient(135deg, #1d467c, #d97706)',
                  border: 'none',
                  padding: '10px 24px',
                  fontWeight: 'bold',
                  color: '#ffffff',
                  boxShadow: '0 4px 12px rgba(29, 70, 124, 0.3)'
                }}
                disabled={submittingRating}
              >
