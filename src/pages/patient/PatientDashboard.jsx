...
function MedicalLeaveTab({ _studentProfileData, user, _currAuthProfile }) {
  ...
  const _initiateProcess = (e) => {
    ...
    if (start > today || end > today) {
      toast.error('You cannot apply for future medical leave dates in advance! ΓÜá∩╕Å');
      return;
    }
    ...
    if (diffDays < 3) {
      toast.error('Medical Leave can only be applied for a minimum of 3 days (e.g. 3, 5, 8, 10 days) as per CU Attendance Policy! ΓÜá∩╕Å');
      return;
    }
    ...
    if (daysSinceLeaveEnd > 10) {
      toast.error(`Exemption window expired! You must submit medical leave within 10 days of your leave ending. (Ended ${daysSinceLeaveEnd} days ago) ΓÜá∩╕Å`);
      return;
    }
  ...
  const handleApplyLeave = (e) => {
    ...
    setTimeout(() => {
      ...
      toast.success(isFaculty ? 'Faculty Medical Leave applied! Exemption pending HOD verification. ≡ƒôæ' : 'Medical Leave applied! Exemption pending warden verification. ≡ƒôæ');
  ...
  const handleDownloadMedicalCertificate = (leaveItem) => {
    try {
      toast.loading('Generating official Medical Certificate PDF...', { id: 'pdf-cert' });
      const doc = new jsPDF();
      ...
      doc.setFontSize(18);
      doc.text('CHANDIGARH UNIVERSITY', pageWidth / 2, 16, { align: 'center' });

      doc.setFontSize(11);
      doc.setFont('Helvetica', 'normal');
      doc.text('OFFICIAL MEDICAL LEAVE EXEMPTION CERTIFICATE', pageWidth / 2, 25, { align: 'center' });
      doc.text('MedAstraQ Verified Health Gateway ΓÇó CU Campus Health Center', pageWidth / 2, 31, { align: 'center' });
      ...
      doc.roundedRect(14, y, pageWidth - 28, 110, 4, 4, 'FD');

      y += 12;
      doc.setTextColor(15, 23, 42);
      doc.setFont('Helvetica', 'bold');
      doc.setFontSize(13);
      doc.text('MEDICAL CERTIFICATE DETAILS', 22, y);

      y += 10;
      doc.setFontSize(10);
      doc.setFont('Helvetica', 'normal');

      const studentName = _currAuthProfile?.name || _studentProfileData?.name || user?.name || 'Naina Kumari';
      const studentUid = _studentProfileData?.collegeUid || _studentProfileData?.uid || '24BCF10013';

      const details = [
        ['Student Name:', studentName],
        ['College UID:', studentUid],
        ['Department:', isFaculty ? 'Faculty Member (CU Academic Branch)' : 'Computer Science & Engineering'],
        ['Leave Period:', `${leaveItem.startDate} to ${leaveItem.endDate}`],
        ['Medical Diagnosis:', leaveItem.reason || 'Medical Illness / Fitness Exemption'],
        ['Certified Physician:', leaveItem.doctor || 'Dr. Aditya Sharma (MD)'],
        ['Verification Status:', 'APPROVED & VERIFIED (CUIMS Exemption Active)'],
        ['Certificate ID:', `CU-MED-LEAVE-${leaveItem.id || Date.now()}`]
      ];
      ...
      doc.roundedRect(14, y, pageWidth - 28, 30, 4, 4, 'FD');

      doc.setTextColor(6, 95, 70);
      doc.setFont('Helvetica', 'bold');
      doc.setFontSize(10);
      doc.text('Γ£ö∩╕Å VERIFIED & DIGITALLY SIGNED BY CU HEALTH OFFICER', 22, y + 12);
      doc.setFontSize(8.5);
      doc.setFont('Helvetica', 'normal');
      doc.text('This medical certificate is generated by MedAstraQ Health Portal & valid for CUIMS attendance exemption.', 22, y + 21);

      y += 45;
      doc.setFontSize(8);
      doc.setTextColor(148, 163, 184);
      doc.text(`Generated on ${new Date().toLocaleDateString()} via MedAstraQ Platform. Document Reference: ${leaveItem.file}`, pageWidth / 2, y, { align: 'center' });

      const fileName = leaveItem.file || `Medical_Certificate_${leaveItem.id || 'leave'}.pdf`;
      doc.save(fileName);
      toast.success(`Downloaded: ${fileName} ≡ƒôä`, { id: 'pdf-cert' });
    } catch (err) {
      console.error('Failed to generate PDF:', err);
      toast.error('Failed to download PDF. Please try again.', { id: 'pdf-cert' });
    }
  };
  ...
  return (
    <div className="cuims-medical-leave-container" style={{ fontFamily: 'Outfit, sans-serif' }}>
          ...
          {!proceeded ? (
            ...
            <div style={{ maxWidth: '600px', margin: '0 auto' }}>
              <h3 style={{ fontSize: '1.1rem', fontWeight: 700, color: '#031B33', borderBottom: '1px solid #e2e8f0', paddingBottom: '10px', marginBottom: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span>≡ƒôï {isFaculty ? 'Faculty Medical Exemption Certificate' : 'Medical Leave Certificate Submission'}</span>
                ...
                <div style={{ background: '#f8fafc', padding: '16px', borderRadius: '12px', border: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  ...
                  <div>
                    <label style={{ display: 'block', fontSize: '0.74rem', fontWeight: 600, color: '#64748b', marginBottom: '2px' }}>
                      ...
                      {prescriptionStatus === 'valid' && <span style={{ marginLeft: '6px', color: '#16a34a', fontWeight: 'bold' }}>≡ƒƒó Valid</span>}
                      {prescriptionStatus === 'invalid' && <span style={{ marginLeft: '6px', color: '#dc2626', fontWeight: 'bold' }}>≡ƒö┤ Invalid format</span>}
                    </label>
                    ...
                    <input 
                      ...
                      onChange={(e) => {
                        ...
                        if (file) {
                          ...
                          if (ext !== 'jpg' && ext !== 'jpeg') {
                            toast.error('Prescription Document must be a JPG/JPEG image! ΓÜá∩╕Å');
                            e.target.value = null;
                            setPrescriptionDoc(null);
                  ...
                  <div>
                    <label style={{ display: 'block', fontSize: '0.74rem', fontWeight: 600, color: '#64748b', marginBottom: '2px' }}>
                      ...
                      {fitnessStatus === 'valid' && <span style={{ marginLeft: '6px', color: '#16a34a', fontWeight: 'bold' }}>≡ƒƒó Valid</span>}
                      {fitnessStatus === 'invalid' && <span style={{ marginLeft: '6px', color: '#dc2626', fontWeight: 'bold' }}>≡ƒö┤ Invalid format</span>}
                    </label>
                    <span style={{ display: 'block', fontSize: '0.68rem', color: '#94a3b8', marginBottom: '4px' }}>(Only JPG/JPEG format allowed)</span>
                    <input 
                      type="file" 
                      ...
                      onChange={(e) => {
                        ...
                        if (file) {
                          const ext = file.name.split('.').pop().toLowerCase();
                          if (ext !== 'jpg' && ext !== 'jpeg') {
                            toast.error('Fitness Certificate must be a JPG/JPEG image! ΓÜá∩╕Å');
                            e.target.value = null;
                            setFitnessCert(null);
                  ...
                  <div>
                    <label style={{ display: 'block', fontSize: '0.74rem', fontWeight: 600, color: '#64748b', marginBottom: '2px' }}>
                      3. Hospital OPD Slip *
                      {opdStatus === 'valid' && <span style={{ marginLeft: '6px', color: '#16a34a', fontWeight: 'bold' }}>≡ƒƒó Valid</span>}
                      {opdStatus === 'invalid' && <span style={{ marginLeft: '6px', color: '#dc2626', fontWeight: 'bold' }}>≡ƒö┤ Invalid format</span>}
                    </label>
                    <span style={{ display: 'block', fontSize: '0.68rem', color: '#94a3b8', marginBottom: '4px' }}>(Only JPG/JPEG format allowed)</span>
                    <input 
                      type="file" 
                      ...
                      onChange={(e) => {
                        ...
                        if (file) {
                          const ext = file.name.split('.').pop().toLowerCase();
                          if (ext !== 'jpg' && ext !== 'jpeg') {
                            toast.error('Hospital OPD Slip must be a JPG/JPEG image! ΓÜá∩╕Å');
                            e.target.value = null;
                            setOpdSlip(null);
            ...
            {leaves.map((l) => (
              <div key={l.id} style={{ border: '1px solid #e2e8f0', borderRadius: '12px', padding: '14px', background: '#f8fafc', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <div style={{ textAlign: 'right' }}>
                  ...
                  <div 
                    onClick={() => handleDownloadMedicalCertificate(l)}
                    style={{ fontSize: '0.72rem', color: '#0284c7', marginTop: '6px', textDecoration: 'underline', cursor: 'pointer' }}
                    title="Click to Download Official Medical Certificate PDF"
                  >
                    ≡ƒôä {l.file}
                  </div>
                </div>
          ...
          <div style={{ background: '#ffffff', maxWidth: '850px', width: '100%', maxHeight: '90vh', borderRadius: '16px', display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.3)', border: '1px solid #cbd5e1' }}>
            ...
            <div style={{ background: 'linear-gradient(135deg, #e11d48, #be123c)', color: '#ffffff', padding: '18px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <h3 style={{ fontSize: '1.25rem', fontWeight: 800, margin: 0, display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <span>≡ƒ⌐║</span> Process to Apply Medical Leave
                </h3>
                <p style={{ fontSize: '0.8rem', opacity: 0.9, margin: '4px 0 0 0' }}>Official CUIMS / MedAstraQ Medical Leave Workflow & Step-by-Step Guide</p>
              </div>
              <button onClick={() => setShowLeaveProcessModal(false)} style={{ background: 'rgba(255,255,255,0.2)', border: 'none', color: '#ffffff', width: '32px', height: '32px', borderRadius: '50%', cursor: 'pointer', fontSize: '1.1rem', fontWeight: 'bold' }}>Γ£ò</button>
            </div>

            {/* Sub-header Bar */}
            <div style={{ display: 'flex', background: '#f8fafc', borderBottom: '1px solid #cbd5e1', padding: '12px 20px' }}>
              <span style={{ fontWeight: 700, fontSize: '0.88rem', color: '#e11d48', display: 'flex', alignItems: 'center', gap: '6px' }}>
                ≡ƒôï Official 11-Step Medical Leave Application Guide
              </span>
            </div>
            ...
            <div style={{ padding: '20px', overflowY: 'auto', flex: 1, fontSize: '0.86rem', color: '#334155' }}>
              <div style={{ background: '#ffe4e6', borderLeft: '4px solid #e11d48', padding: '12px 16px', borderRadius: '4px', marginBottom: '16px', color: '#9f1239', fontWeight: 600 }}>
                ≡ƒôî <strong>Medical Leave Rules:</strong> Student can apply only post-dated leave within <strong>SEVEN days</strong> of reporting back. Minimum duration is <strong>3 working days</strong>. Maximum benefit allowed is <strong>10 working days</strong> per semester.
              </div>
      ...
      {/* ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
          2. LEAVE POLICY MODAL (Medical Leave Only)
      ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.65)', backdropFilter: 'blur(4px)', zIndex: 999999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}>
          <div style={{ background: '#ffffff', maxWidth: '900px', width: '100%', maxHeight: '90vh', borderRadius: '16px', display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.3)', border: '1px solid #cbd5e1' }}>
            ...
            <div style={{ background: 'linear-gradient(135deg, #ea580c, #c2410c)', color: '#ffffff', padding: '18px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <h3 style={{ fontSize: '1.25rem', fontWeight: 800, margin: 0, display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <span>≡ƒô£</span> MEDICAL LEAVE POLICY (Academic Planning & Monitoring)
                </h3>
                <p style={{ fontSize: '0.8rem', opacity: 0.9, margin: '4px 0 0 0' }}>Official Guidelines, Mandatory Verification & Rules for Medical Leave</p>
              </div>
              <button onClick={() => setShowLeavePolicyModal(false)} style={{ background: 'rgba(255,255,255,0.2)', border: 'none', color: '#ffffff', width: '32px', height: '32px', borderRadius: '50%', cursor: 'pointer', fontSize: '1.1rem', fontWeight: 'bold' }}>Γ£ò</button>
            </div>
            ...
            <div style={{ display: 'flex', background: '#f8fafc', borderBottom: '1px solid #cbd5e1', padding: '0 16px' }}>
              <button 
                ...
                style={{ padding: '12px 18px', border: 'none', background: 'transparent', fontWeight: 700, fontSize: '0.85rem', color: policySubTab === 'medical' ? '#ea580c' : '#64748b', borderBottom: policySubTab === 'medical' ? '3px solid #ea580c' : 'none', cursor: 'pointer' }}
              >
                ≡ƒ⌐║ Medical Leave Policy Rules
              </button>
              <button 
                onClick={() => setPolicySubTab('abbreviations')}
                style={{ padding: '12px 18px', border: 'none', background: 'transparent', fontWeight: 700, fontSize: '0.85rem', color: policySubTab === 'abbreviations' ? '#ea580c' : '#64748b', borderBottom: policySubTab === 'abbreviations' ? '3px solid #ea580c' : 'none', cursor: 'pointer' }}
              >
                ≡ƒöñ Medical Abbreviations Index
              </button>
            </div>
            ...
            <div style={{ padding: '20px', overflowY: 'auto', flex: 1, fontSize: '0.86rem', color: '#334155' }}>
                ...
                <div>
                  <div style={{ background: '#fff7ed', borderLeft: '4px solid #ea580c', padding: '12px 16px', borderRadius: '4px', marginBottom: '16px' }}>
                    ...
                    <p style={{ margin: 0, fontSize: '0.83rem', color: '#9a3412' }}>
                      The provision of medical leave is provided ONLY in exceptional cases of serious illness during the semester. Students must maintain a minimum of <strong>75% attendance</strong> in each subject in the NON-ML period as per UGC guidelines. Minor illnesses must be covered under the 25% relaxation window.
                    </p>
                  </div>
              ...
              {policySubTab === 'abbreviations' && (
                <div>
                  <h4 style={{ fontSize: '0.95rem', fontWeight: 800, color: '#1e293b', marginBottom: '10px' }}>Official Medical Leave Abbreviations Index</h4>
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.83rem' }}>
                    ...
                    <tbody>
                      <tr style={{ borderBottom: '1px solid #e2e8f0' }}><td style={{ padding: '8px 12px', fontWeight: 700, color: '#ea580c' }}>ML</td><td style={{ padding: '8px 12px' }}>Medical Leave</td></tr>
      ...
      {/* ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
          3. FAQ'S MODAL (Medical Leave Only)
      ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
      {showFaqsModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.65)', backdropFilter: 'blur(4px)', zIndex: 999999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}>
          <div style={{ background: '#ffffff', maxWidth: '850px', width: '100%', maxHeight: '90vh', borderRadius: '16px', display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.3)', border: '1px solid #cbd5e1' }}>
            ...
            <div style={{ background: 'linear-gradient(135deg, #0f766e, #115e59)', color: '#ffffff', padding: '18px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <h3 style={{ fontSize: '1.25rem', fontWeight: 800, margin: 0, display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <span>≡ƒ⌐║</span> MEDICAL LEAVE FAQ'S
                </h3>
                <p style={{ fontSize: '0.8rem', opacity: 0.9, margin: '4px 0 0 0' }}>Search Frequently Asked Questions regarding Medical Leave Guidelines & Rules</p>
              </div>
              <button onClick={() => setShowFaqsModal(false)} style={{ background: 'rgba(255,255,255,0.2)', border: 'none', color: '#ffffff', width: '32px', height: '32px', borderRadius: '50%', cursor: 'pointer', fontSize: '1.1rem', fontWeight: 'bold' }}>Γ£ò</button>
            </div>
            ...
            <div style={{ padding: '14px 20px', background: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
              <input 
                type="text" 
                placeholder="≡ƒöì Search Medical Leave FAQs by keyword (e.g. 3 days, documents, board, attendance)..." 
            ...
            <div style={{ padding: '20px', overflowY: 'auto', flex: 1, display: 'flex', flexDirection: 'column', gap: '14px' }}>
              {(() => {
                const faqsList = [
                  { q: "Q1: Can I apply the medical leave for less than three days?", a: "No, student can only apply the medical leave for minimum of 3 days and the final approval shall lie with the medical board only.", category: "Medical Leave" },
                  { q: "Q2: How can I apply medical leave?", a: "You have to apply for medical leave through CUIMS / MedAstraQ within SEVEN days after returning from the period of sickness.", category: "Medical Leave" },
                  { q: "Q3: Who will approve my medical leave?", a: "All the medical leaves shall be approved by Medical Board of the university under the Registrar office.", category: "Medical Leave" },
                  { q: "Q4: What all documents I need to bring while applying for medical leave?", a: "After applying online leave within seven days after returning from the leave of illness. During physical verification a student need to bring all proper medical documents like Prescription of doctor, admission in hospital, blood reports, test reports, discharge slip, doctor fitness certificate, and medical bills.", category: "Medical Leave" },
                  { q: "Q5: How many medical leaves can I avail in a semester?", a: "A student can take maximum of 10 working days medical leave in a semester.", category: "Medical Leave" },
                  { q: "Q6: What if student is ill for period of more than 10 working days?", a: "In this case student need to submit the application/form along with all documents to respective cluster Executive Director, but the medical should be applied from student and approved by medical board on CUIMS. This is a request form which is subjective to approval by the panel of authorities decided by office of Vice Chancellor.", category: "Medical Leave" },
                  { q: "Q7: What does NON-ML period mean for attendance calculation?", a: "NON-ML period means Non-Medical Leave period. For e.g.:- If 40 lectures were delivered and you took 5 lectures ML, then Non-ML period is 35 lectures. Student must maintain at-least 75% attendance in NON-ML period in order to be eligible to appear in exams.", category: "Medical Leave" },
                  { q: "Q8: I am a TPP registered student. What is my Medical Leave criteria?", a: "Medical Leave benefit remains the same as for normal student (maximum 10 working days per semester), but the criteria of eligibility in examination is decided by the Department of Career Development (DCPD).", category: "Medical Leave" }
                ];

                const filtered = faqsList.filter(item => {
                  return !faqSearchQuery || item.q.toLowerCase().includes(faqSearchQuery.toLowerCase()) || item.a.toLowerCase().includes(faqSearchQuery.toLowerCase());
                });

                if (filtered.length === 0) {
                  return (
                    <div style={{ textAlign: 'center', padding: '40px 20px', color: '#64748b' }}>
                      <span style={{ fontSize: '2rem' }}>≡ƒöì</span>
                      <p style={{ marginTop: '8px', fontWeight: 600 }}>No Medical Leave FAQs found matching "{faqSearchQuery}"</p>
                    </div>
                  );
                }
...
function FacultyPortalTab({ _studentProfileData, fetchBookings }) {
  ...
  const handleBookCheckup = async (e) => {
    ...
    try {
      ...
      toast.success(`Occupational Full Body Checkup (${frequency === 'monthly' ? 'Monthly' : '2-Monthly'}) booked successfully! ≡ƒÅÑ`);
      setDate('');
  ...
  const handleClaimInsurance = () => {
    ...
    toast.success('Corporate Health Insurance Activated successfully! ≡ƒ¢í∩╕Å');
  };
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px', alignItems: 'start' }}>
        ...
        <div style={{ background: '#f8fafc', border: '1px solid #cbd5e1', borderRadius: '16px', padding: '24px', boxShadow: '0 4px 12px rgba(0,0,0,0.02)' }}>
          <h3 style={{ fontSize: '1.15rem', color: '#0f766e', fontWeight: 700, borderBottom: '1px solid #e2e8f0', paddingBottom: '12px', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '8px' }}>
            ≡ƒÆ╝ Faculty Health Benefits
          </h3>
          
          <ul style={{ paddingLeft: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '14px', margin: '0 0 20px 0' }}>
            <li style={{ display: 'flex', gap: '12px' }}>
              <span style={{ fontSize: '1.2rem' }}>≡ƒ¢í∩╕Å</span>
              <div>
                <strong style={{ display: 'block', fontSize: '0.88rem', color: '#1e293b' }}>Corporate Health Insurance</strong>
                <span style={{ fontSize: '0.8rem', color: '#64748b' }}>Coverage up to Γé╣5,00,000 for self, spouse, and dependents.</span>
              </div>
            </li>
            <li style={{ display: 'flex', gap: '12px' }}>
              <span style={{ fontSize: '1.2rem' }}>≡ƒÅÑ</span>
              <div>
            ...
            <li style={{ display: 'flex', gap: '12px' }}>
              <span style={{ fontSize: '1.2rem' }}>≡ƒÆè</span>
              <div>
            ...
            <li style={{ display: 'flex', gap: '12px' }}>
              <span style={{ fontSize: '1.2rem' }}>≡ƒºÿ</span>
              <div>
            ...
            <li style={{ display: 'flex', gap: '12px' }}>
              ...
              <div>
                ...
                <span style={{ fontSize: '0.8rem', color: '#64748b' }}>Reimbursement up to Γé╣25,000 for standing desks, ergonomic chairs, or fitness smartwatches.</span>
              </div>
            </li>
            <li style={{ display: 'flex', gap: '12px' }}>
              <span style={{ fontSize: '1.2rem' }}>≡ƒª╖</span>
              <div>
                <strong style={{ display: 'block', fontSize: '0.88rem', color: '#1e293b' }}>Biannual Dental & Vision Package</strong>
                <span style={{ fontSize: '0.8rem', color: '#64748b' }}>Free teeth polishing & eye exams plus Γé╣5,000 annual frame allowance.</span>
              </div>
            </li>
            <li style={{ display: 'flex', gap: '12px' }}>
              <span style={{ fontSize: '1.2rem' }}>≡ƒÅï∩╕Å</span>
              <div>
          ...
          >
            ≡ƒÄü Open Faculty Health Benefits Portal
          </button>
        </div>
        ...
        <div style={{ background: '#ffffff', border: '1px solid #cbd5e1', borderRadius: '16px', padding: '24px', boxShadow: '0 4px 12px rgba(0,0,0,0.02)' }}>
          <h3 style={{ fontSize: '1.15rem', color: '#1e3a5f', fontWeight: 700, borderBottom: '1px solid #e2e8f0', paddingBottom: '12px', marginBottom: '16px' }}>
            ≡ƒôà Book Full Body Occupational Checkup
          </h3>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: '16px' }}>
          <div style={{ background: '#ffffff', borderRadius: '16px', maxWidth: '500px', width: '100%', padding: '24px', boxShadow: '0 20px 40px rgba(0,0,0,0.25)', border: '1px solid #e2e8f0', position: 'relative' }}>
            ...
            <div style={{ textAlign: 'center', marginBottom: '16px' }}>
              <span style={{ fontSize: '2.5rem' }}>≡ƒ¢í∩╕Å</span>
            ...
            <div style={{ background: '#f8fafc', padding: '16px', borderRadius: '12px', border: '1px solid #cbd5e1', marginBottom: '20px' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px', fontSize: '0.82rem' }}>
                ...
                <strong style={{ color: '#1e293b' }}>Γé╣5,00,000 INR</strong>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px', fontSize: '0.82rem' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px', fontSize: '0.82rem' }}>
                ...
                <strong style={{ color: '#1e293b' }}>Γé╣25,000 allowance active</strong>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px', fontSize: '0.82rem' }}>
                <span style={{ color: '#64748b' }}>Dental & Vision:</span>
                <strong style={{ color: '#1e293b' }}>Free Checkups + Γé╣5,000 Frame</strong>
              </div>
            ...
            {insuranceActivated ? (
              <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', padding: '12px 16px', borderRadius: '8px', textAlign: 'center', marginBottom: '16px' }}>
                <span style={{ color: '#15803d', fontSize: '0.85rem', fontWeight: 700, display: 'block' }}>Γ£ô Insurance Benefit Active</span>
...
function MainDashboardPanel(props) {
  ...
  const toggleTheme = () => {
    ...
    toast.success(`Switched to ${newTheme === 'dark' ? 'Dark ≡ƒîÖ' : 'Light ΓÿÇ∩╕Å'} mode`);
  };
          ...
          async (pos) => {
            try {
              ...
              if (weatherData && weatherData.current_weather) {
                ...
                let cond = 'clear sky';
                let icon = 'ΓÿÇ∩╕Å';
                if (code >= 1 && code <= 3) { cond = 'partly cloudy'; icon = 'Γ¢à'; }
                else if (code >= 45 && code <= 48) { cond = 'foggy'; icon = '≡ƒî½∩╕Å'; }
                else if (code >= 51 && code <= 67) { cond = 'rainy'; icon = '≡ƒîº∩╕Å'; }
                else if (code >= 80 && code <= 99) { cond = 'thunderstorm'; icon = 'Γ¢ê∩╕Å'; }
  ...
  const handleDownloadVirtualIDCard = async () => {
    ...
    doc.setFontSize(5.5);
    doc.setTextColor(255, 255, 255);
    doc.text('Verified by University Health Center   |   Secure ΓÇó Encrypted', 42.8, 132.5, { align: 'center' });
    ...
    toast.success('Your 2-sided Student Health ID Card PDF has been downloaded successfully! ≡ƒô▒≡ƒÆÜ');
  } catch (err) {
  ...
  const [chatHistory, setChatHistory] = useState([
    {
      sender: 'ai',
      text: '≡ƒæï **Hello! I am Astra, your AI-powered medical assistant.**\n\nI provide evidence-based health guidance and can help you:\n- ≡ƒ⌐║ Analyze symptoms with clinical triage\n- ≡ƒÆè Suggest wellness & prevention tips\n- ≡ƒæ¿ΓÇìΓÜò∩╕Å Recommend the right specialist doctor\n\n*Tell me what symptoms you\'re experiencing, and I\'ll guide you step by step.*'
    }
  ]);
  ...
  const handleToggleVoiceListening = () => {
    ...
    try {
      ...
      rec.onstart = () => {
        ...
        toast.success('≡ƒÄÖ∩╕Å Listening for symptoms... Speak now!', { id: 'voice-active' });
      };
  ...
  const quickTags = [
    { label: '≡ƒù║∩╕Å 2D Body Map', isAction: true },
    { label: '≡ƒñò Headache', query: 'I have a headache that started today. Can you help me assess it?' },
    { label: '≡ƒîí∩╕Å Fever', query: 'I have a fever. What should I check and when should I see a doctor?' },
    { label: '≡ƒÑù Diet Plan', query: 'Can you suggest a healthy balanced diet plan for overall wellness?' },
    { label: '≡ƒôà Book Doctor', query: 'I want to book a doctor appointment. What specialists are available?' },
    { label: '≡ƒÿ╖ Cold & Cough', query: 'I have cold and cough symptoms. Is this something serious?' }
  ];
  ...
  const handleSendChat = async (textToSend) => {
    ...
    try {
      ...
      console.error(err);
      setChatHistory(prev => [...prev, { sender: 'ai', text: 'ΓÜá∩╕Å **Connection Error:** Could not reach Astra. Please check your internet connection and try again.' }]);
    } finally {
      setSendingChat(false);
      setTimeout(() => {
        const chatBody = document.getElementById('chat-body');
  ...
  const handleResetChat = async () => {
    ...
    setChatHistory([
      {
        sender: 'ai',
        text: '≡ƒæï **Conversation reset!** I\'m ready for a fresh consultation.\n\n*Tell me what symptoms you\'re experiencing, and I\'ll guide you step by step.*'
      }
    ]);
  };
  ...
  const fetchAdvisories = async () => {
    try {
      const res = await hospitalAPI.getAdvisories();
    ...
    return (
      <div className="cuims-card analytics-card animate-slide-up" style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '24px', marginTop: '20px', background: '#f8fafc', borderRadius: '18px', border: 'none', color: '#1e293b', fontFamily: 'Outfit, sans-serif' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
          <div>
            <h1 style={{ margin: 0, fontSize: '1.25rem', fontWeight: 800, color: '#1e293b', display: 'flex', alignItems: 'center', gap: '6px' }}>
              Welcome, {_currAuthProfile?.name || _studentProfileData?.name || 'Rashika'} ≡ƒæï
            </h1>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', position: 'relative' }}>
            ...
            <div style={{ position: 'relative' }}>
              ...
              >
                ...
                <span style={{ fontSize: '0.62rem', color: '#64748b' }}>Γû╝</span>
              </div>
              ...
              {dateRangeDropdownOpen && (
                <div style={{
                  ...
                  >
                    Γ£¿ 01 Jan {selectedYear} - 31 Dec {selectedYear} (Full Year)
                  </div>
                  <div 
                    onClick={() => {
                  ...
                  >
                    ≡ƒôà 01 Jan {selectedYear} - 31 Jul {selectedYear} (Jan - Jul)
                  </div>
                </div>
              )}
            </div>
                ...
                <div style={{
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                    <div style={{ display: 'flex', gap: '10px', fontSize: '0.74rem' }}>
                      <span style={{ fontSize: '1rem' }}>≡ƒôó</span>
                    ...
                    <div style={{ display: 'flex', gap: '10px', fontSize: '0.74rem', borderTop: '1px solid #f8fafc', paddingTop: '8px' }}>
                      <span style={{ fontSize: '1rem' }}>≡ƒö¼</span>
                      <div style={{ textAlign: 'left' }}>
                    ...
                    <div style={{ display: 'flex', gap: '10px', fontSize: '0.74rem', borderTop: '1px solid #f8fafc', paddingTop: '8px' }}>
                      <span style={{ fontSize: '1rem' }}>Γ£à</span>
                      <div style={{ textAlign: 'left' }}>
              ...
              <div style={{ background: '#ffffff', borderRadius: '18px', border: '1px solid #e2e8f0', padding: '24px', display: 'flex', flexDirection: 'column', boxShadow: '0 1px 3px rgba(0,0,0,0.02)' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #f1f5f9', paddingBottom: '16px', marginBottom: '20px', flexWrap: 'wrap', gap: '12px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                    ...
                    <span style={{ cursor: 'pointer', color: '#94a3b8', fontSize: '0.85rem' }} title="Shows your scheduled doctor visits on a monthly calendar sheet">Γôÿ</span>
                  </div>
                ...
                <div style={{ 
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #f1f5f9', paddingBottom: '10px' }}>
                    <strong style={{ fontSize: '0.86rem', color: '#1e293b' }}>
                      ≡ƒùô∩╕Å Appointments on {selectedCalendarDate.toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' })}
                    </strong>
                    <button 
                      type="button" 
        ...
        {analyticsChartType === 'weekly' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', background: '#ffffff', padding: '20px', borderRadius: '18px', border: '1px solid #e2e8f0', boxShadow: '0 1px 3px rgba(0,0,0,0.02)' }}>
            <h3 style={{ margin: 0, fontSize: '0.92rem', color: '#1e293b', fontWeight: 800 }}>
              ≡ƒôè Weekly Appointments Count (Last 12 Weeks)
            </h3>
        ...
        {analyticsChartType === 'monthly' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', background: '#ffffff', padding: '20px', borderRadius: '18px', border: '1px solid #e2e8f0', boxShadow: '0 1px 3px rgba(0,0,0,0.02)' }}>
            <h3 style={{ margin: 0, fontSize: '0.92rem', color: '#1e293b', fontWeight: 800 }}>
              ≡ƒôê Monthly Visits Velocity (Last 6 Months)
            </h3>
  ...
  const renderMedicineTrendsDashboard = () => {
    const runMedGemmaAudit = () => {
      ...
      setTimeout(() => {
        setMedGemmaReport({
          status: 'No critical drug interactions found! Γ£à',
          details: [
      ...
      <div className="cuims-card medicine-trends-card animate-slide-up" style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '24px', marginTop: '20px', background: '#f8fafc', borderRadius: '18px', border: 'none', color: '#1e293b', fontFamily: 'Outfit, sans-serif' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '16px' }}>
          ...
          <div style={{ background: '#ffffff', borderRadius: '16px', border: '1px solid #e2e8f0', padding: '16px', display: 'flex', alignItems: 'center', gap: '14px' }}>
            <div style={{ width: '48px', height: '48px', borderRadius: '50%', background: 'rgba(4, 42, 89, 0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.25rem' }}>≡ƒ⌐║</div>
            <div>
              <div style={{ fontSize: '0.74rem', color: '#64748b', fontWeight: 600 }}>Total Visits</div>
          ...
          {/* Card 2: Checkups */}
          <div style={{ background: '#ffffff', borderRadius: '16px', border: '1px solid #e2e8f0', padding: '16px', display: 'flex', alignItems: 'center', gap: '14px' }}>
            <div style={{ width: '48px', height: '48px', borderRadius: '50%', background: 'rgba(34, 197, 94, 0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.25rem' }}>≡ƒƒó</div>
            <div>
              <div style={{ fontSize: '0.74rem', color: '#64748b', fontWeight: 600 }}>Checkups</div>
          ...
          {/* Card 3: Follow-ups */}
          <div style={{ background: '#ffffff', borderRadius: '16px', border: '1px solid #e2e8f0', padding: '16px', display: 'flex', alignItems: 'center', gap: '14px' }}>
            <div style={{ width: '48px', height: '48px', borderRadius: '50%', background: 'rgba(29, 70, 124, 0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.25rem' }}>≡ƒòÆ</div>
            <div>
              <div style={{ fontSize: '0.74rem', color: '#64748b', fontWeight: 600 }}>Follow-ups</div>
          ...
          {/* Card 4: Emergency */}
          <div style={{ background: '#ffffff', borderRadius: '16px', border: '1px solid #e2e8f0', padding: '16px', display: 'flex', alignItems: 'center', gap: '14px' }}>
            <div style={{ width: '48px', height: '48px', borderRadius: '50%', background: 'rgba(239, 68, 68, 0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.25rem' }}>≡ƒÜ¿</div>
            <div>
              <div style={{ fontSize: '0.74rem', color: '#64748b', fontWeight: 600 }}>Emergency</div>
          ...
          {/* Card 5: Lab Tests */}
          <div style={{ background: '#ffffff', borderRadius: '16px', border: '1px solid #e2e8f0', padding: '16px', display: 'flex', alignItems: 'center', gap: '14px' }}>
            <div style={{ width: '48px', height: '48px', borderRadius: '50%', background: 'rgba(4, 42, 89, 0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.25rem' }}>≡ƒº¬</div>
            <div>
              <div style={{ fontSize: '0.74rem', color: '#64748b', fontWeight: 600 }}>Lab Tests</div>
          ...
          <div style={{ padding: '24px', background: '#ffffff', borderRadius: '18px', border: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', gap: '16px', justifyContext: 'space-between', justifyContent: 'space-between' }}>
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                ...
                <span style={{ color: '#94a3b8', fontSize: '0.8rem', cursor: 'pointer' }} title="Shows repeat prescriptions vs new medicines prescribed">Γôÿ</span>
              </div>
            </div>
        ...
        <div style={{ padding: '20px', background: '#ffffff', borderRadius: '18px', border: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', gap: '16px', boxShadow: '0 1px 3px rgba(0,0,0,0.02)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #f1f5f9', paddingBottom: '12px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <span style={{ fontSize: '1.3rem' }}>≡ƒñû</span>
              <div>
                <h3 style={{ margin: 0, fontSize: '0.95rem', fontWeight: 800, color: '#1e293b' }}>
            ...
            >
              <span>{medGemmaLoading ? 'Inference Running...' : 'ΓÜÖ∩╕Å Run MedGemma AI Audit'}</span>
            </button>
          </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '14px' }}>
                {medGemmaReport.details.map((item, idx) => (
                  <div key={idx} style={{ background: '#ffffff', padding: '12px', borderRadius: '8px', border: '1px solid #f1f5f9', display: 'flex', flexDirection: 'column', gap: '4px' }}>
                    <span style={{ fontWeight: 800, color: '#1e293b', fontSize: '0.78rem' }}>≡ƒÆè {item.med}</span>
        ...
        <div style={{ fontSize: '0.72rem', color: '#94a3b8', textAlign: 'left', fontWeight: 500 }}>
          Γôÿ Note: Trends are based on prescriptions from selected date range.
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="cuims-card wellness-score-card animate-slide-up" style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '24px', marginTop: '20px', background: '#f8fafc', borderRadius: '18px', border: 'none', color: '#1e293b', fontFamily: 'Outfit, sans-serif' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px', borderBottom: '1px solid #e2e8f0', paddingBottom: '16px' }}>
          <div>
            <h1 style={{ margin: 0, fontSize: '1.4rem', fontWeight: 800, color: '#1e293b', display: 'flex', alignItems: 'center', gap: '8px' }}>
              Γ¥ñ∩╕Å Student Wellness Score Dashboard
            </h1>
          ...
          <div style={{ background: '#ffffff', border: '1px solid #e2e8f0', padding: '6px 14px', borderRadius: '20px', fontSize: '0.78rem', color: '#334155', fontWeight: 700 }}>
            <span>≡ƒöÆ Local WebGL Offline Calculations</span>
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px' }}>
          <div style={{ background: '#ffffff', borderRadius: '16px', border: '1px solid #e2e8f0', padding: '16px', display: 'flex', alignItems: 'center', gap: '14px' }}>
            <div style={{ width: '48px', height: '48px', borderRadius: '50%', background: 'rgba(34, 197, 94, 0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.25rem' }}>≡ƒ¢í∩╕Å</div>
            <div>
          ...
          <div style={{ background: '#ffffff', borderRadius: '16px', border: '1px solid #e2e8f0', padding: '16px', display: 'flex', alignItems: 'center', gap: '14px' }}>
            <div style={{ width: '48px', height: '48px', borderRadius: '50%', background: 'rgba(4, 42, 89, 0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.25rem' }}>≡ƒôà</div>
            <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: '24px' }}>
          ...
          <div style={{ padding: '24px', background: '#ffffff', borderRadius: '18px', border: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', gap: '16px', boxShadow: '0 1px 3px rgba(0,0,0,0.02)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                ...
                <span style={{ cursor: 'pointer', color: '#94a3b8', fontSize: '0.85rem' }} title="Shows your monthly wellness index recovery progress">Γôÿ</span>
              </div>
          ...
          <div style={{ padding: '24px', background: '#ffffff', borderRadius: '18px', border: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', gap: '16px', boxShadow: '0 1px 3px rgba(0,0,0,0.02)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', borderBottom: '1px solid #f1f5f9', paddingBottom: '12px', marginBottom: '4px' }}>
              ...
              <span style={{ cursor: 'pointer', color: '#94a3b8', fontSize: '0.85rem' }} title="AI suggestions derived from checkups and habits">Γôÿ</span>
            </div>
        ...
        <div style={{ padding: '20px', background: '#ffffff', borderRadius: '18px', border: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', gap: '14px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', borderBottom: '1px solid #f1f5f9', paddingBottom: '10px' }}>
            <span style={{ fontSize: '1.25rem' }}>≡ƒñû</span>
            <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '12px', fontSize: '0.76rem' }}>
            <div style={{ background: '#fdfbf7', border: '1px solid #fef3c7', padding: '12px', borderRadius: '8px' }}>
              <strong style={{ color: '#d97706', display: 'block', marginBottom: '4px' }}>≡ƒì▓ Diet & Recovery</strong>
            ...
            <div style={{ background: '#f5fbf8', border: '1px solid #d1fae5', padding: '12px', borderRadius: '8px' }}>
              <strong style={{ color: '#009091', display: 'block', marginBottom: '4px' }}>≡ƒÆñ Sleep Hygiene</strong>
            ...
            <div style={{ background: '#f0f9ff', border: '1px solid #e0f2fe', padding: '12px', borderRadius: '8px' }}>
              <strong style={{ color: '#0284c7', display: 'block', marginBottom: '4px' }}>≡ƒÅÑ Consult Schedule</strong>
    ...
    <div className="cuims-layout animate-fade-in">
                  ...
                  <div key={item.id} style={{ display: 'flex', flexDirection: 'column', width: '100%' }}>
                    ...
                    {wellnessDropdownOpen && !sidebarCollapsed && (
                      <>
                        ...
                        >
                          <span style={{ fontSize: '0.9rem', color: '#ffffff', fontWeight: 'bold' }}>Γå│</span>
              ...
              <div className="cuims-top-cards-row">
                ...
                <div className="cuims-quick-card lms-card">
                  ...
                  <div className="cuims-quick-card-footer">
                    ...
                    <div className="cuims-quick-card-icon">≡ƒ⌐║</div>
                  </div>
                </div>
                ...
                <div className="cuims-quick-card">
                  ...
                  <div className="cuims-quick-card-footer">
                    ...
                    <div className="cuims-quick-card-icon" style={{ background: '#fee2e2', color: '#ef4444' }}>≡ƒÜ¿</div>
                  </div>
                </div>
              </div>
              ...
              <div className="cuims-banner-row">
                <div className="cuims-weather-pill">
                  ...
                  <div className="cuims-weather-temp">
                    <span>{userCityWeather?.icon || 'Γÿü∩╕Å'} {userCityWeather?.temp || '28┬░C'}</span>
          ...
          <div className="cuims-three-col-grid">
              ...
              <div className="cuims-message-box">
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '10px' }}>
                  ...
                  <span className={`health-badge ${_studentProfileData?.healthBadge || 'STABLE'}`}>
                    {_studentProfileData?.healthBadge === 'CRITICAL' ? '≡ƒö┤ Critical' : _studentProfileData?.healthBadge === 'MONITORING' ? '≡ƒƒí Monitoring' : '≡ƒƒó Stable'}
                  </span>
                </div>
            ...
            <div className="cuims-card">
              <div className="cuims-card-header" style={{ color: '#dc2626' }}>
                <span>≡ƒÜ¿ Emergency SOS & Alerts</span>
              </div>
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontWeight: 700, fontSize: '0.9rem' }}>
                  <span>≡ƒÜ¿</span> 24/7 Campus Emergency SOS
                </div>
              ...
              {advisories && advisories.length > 0 ? (
                advisories.map((advisory) => (
                  <div key={advisory.id} className="cuims-announcement-card" style={{ marginTop: '8px' }}>
                    <div className="cuims-announcement-title" style={{ fontSize: '0.82rem' }}>
                      <span>≡ƒôî</span> {advisory.title}
                    </div>
                    <div className="cuims-announcement-date">
                      <span>≡ƒôà {advisory.date}</span>
                    </div>
                ...
                <div className="cuims-announcement-card" style={{ marginTop: '8px' }}>
                  <p style={{ fontSize: '0.78rem', color: '#475569', margin: 0, lineHeight: 1.3, textAlign: 'center' }}>
                    No active health advisories at the moment.
                  </p>
                </div>
              )}
            </div>
            ...
            <>
              ...
              <div className="filters-row animate-fade-in" style={{ 
                ...
                <div style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: '8px', zIndex: 100 }}>
                  ...
                  >
                    <span>
                      {sortBy === 'rating' ? 'Γ¡É Rating' : 
                       sortBy === 'distance' ? '≡ƒôì Distance' : 
                       sortBy === 'price-asc' ? 'Γé╣ Price: Low to High' : 
                       sortBy === 'price-desc' ? 'Γé╣ Price: High to Low' : 
                       'Default'}
                    </span>
                    <span style={{ fontSize: '0.7rem', opacity: 0.7 }}>Γû╝</span>
                  </button>
                  ...
                  {sortDropdownOpen && (
                    <>
                      ...
                      <div style={{
                        ...
                        {[
                          { value: '', label: 'Default' },
                          { value: 'rating', label: 'Γ¡É Rating' },
                          { value: 'distance', label: '≡ƒôì Distance' },
                          { value: 'price-asc', label: 'Γé╣ Price: Low to High' },
                          { value: 'price-desc', label: 'Γé╣ Price: High to Low' }
                        ].map((item) => (
                          <button
                            key={item.value}
                            type="button"
                        ...
                        <div style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '16px', justifyContent: 'space-between' }}>
                          ...
                          {userCoords && hospital.latitude && hospital.longitude && (
                            <span style={{ 
                              ...
                              ≡ƒôì {getDistance(userCoords.latitude, userCoords.longitude, hospital.latitude, hospital.longitude).toFixed(1)} km
                            </span>
                          )}
                        </div>
                        ...
                        <div style={{ marginTop: 'auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '10px', paddingTop: '16px', borderTop: '1px solid var(--border-color)', flexWrap: 'wrap' }}>
                          <div style={{ flexShrink: 0, minWidth: '95px' }}>
                            ...
                            <span style={{ color: 'var(--primary)', fontWeight: '700', fontSize: '1.05rem', whiteSpace: 'nowrap' }}>
                              {hospital.consultationRate === 0 || hospital.consultationFee === 0 ? 'FREE' : `Γé╣${hospital.consultationRate || hospital.consultationFee || 250}`}
                            </span>
                          </div>
                      ...
                      >
                        ...
                        <div>
                          <span className={`badge ${booking.type === 'ONLINE' ? 'badge-primary' : 'badge-ghost'}`} style={{ fontSize: '0.72rem', padding: '4px 8px' }}>
                            {booking.type === 'ONLINE' ? '≡ƒÄÑ Video' : '≡ƒÅÑ In-Person'}
                          </span>
                        </div>
          ...
          >
            ...
            <div style={{ padding: '32px' }}>
              ...
              <div style={{ display: 'flex', gap: '16px', marginBottom: '24px' }}>
                ...
                <div style={{ padding: '12px', background: 'rgba(var(--primary-rgb), 0.1)', borderRadius: '12px', flex: 1, textAlign: 'center' }}>
                  <div style={{ color: 'var(--primary)', fontWeight: 'bold', fontSize: '1.2rem' }}>Γé╣{_chosenHospitalNode.consultationRate}</div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1100 }} onClick={() => setShowMapModal(false)}>
          <div style={{ width: '90%', maxWidth: '900px', height: '70vh', background: 'white', borderRadius: '12px', overflow: 'hidden' }} onClick={(e) => e.stopPropagation()}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 12px', borderBottom: '1px solid var(--border-color)' }}>
              <div style={{ fontWeight: 700 }}>{_chosenHospitalNode.name} ΓÇö Map</div>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                ...
                <button 
                  onClick={() => {
                    ...
                    toast.success('Report copied to clipboard! ≡ƒôï');
                  }} 
                  className="btn btn-outline"
                  style={{ display: 'flex', alignItems: 'center', gap: '6px' }}
                >
              ...
              <div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '10px', fontSize: '0.82rem' }}>
                  <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', padding: '10px', borderRadius: '10px', textAlign: 'center' }}>
                    ...
                    <div style={{ fontWeight: 800, color: '#15803d', marginTop: '2px' }}>≡ƒƒó STABLE</div>
                  </div>
      ...
      {showSettingsModal && (
        ...
        >
          ...
          >
            {/* Header Banner */}
            <div style={{ background: 'linear-gradient(135deg, #4f46e5, #4338ca)', padding: '24px', color: '#ffffff', position: 'relative' }}>
              <button 
                onClick={() => setShowSettingsModal(false)}
                style={{
                  position: 'absolute', top: '16px', right: '16px',
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                ...
                <div>
                  <h2 style={{ margin: 0, fontSize: '1.4rem', fontWeight: 700 }}>Account Settings</h2>
            ...
            <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '24px', maxHeight: '70vh', overflowY: 'auto' }}>
              ...
              <div style={{ border: '1px solid #e2e8f0', borderRadius: '12px', padding: '16px' }}>
                <h4 style={{ fontSize: '1rem', color: '#1e293b', fontWeight: 600, margin: '0 0 16px 0', display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <FiLock color="#4f46e5" /> Security: Change Password
                </h4>
                <form 
                  onSubmit={(e) => { 
                    e.preventDefault(); 
                    toast.success('Password changed successfully!', { icon: '≡ƒöÉ' }); 
                    e.target.reset();
                  }}
                  style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}
                >
                  <div>
                    <label style={{ display: 'block', fontSize: '0.82rem', color: '#64748b', marginBottom: '4px' }}>Current Password</label>
                    <input type="password" required style={{ width: '100%', padding: '10px 14px', borderRadius: '8px', border: '1px solid #cbd5e1', outline: 'none' }} placeholder="Enter current password" />
                  </div>
                  <div>
                    <label style={{ display: 'block', fontSize: '0.82rem', color: '#64748b', marginBottom: '4px' }}>New Password</label>
                    <input type="password" required style={{ width: '100%', padding: '10px 14px', borderRadius: '8px', border: '1px solid #cbd5e1', outline: 'none' }} placeholder="Enter new password" />
                  </div>
                  <div>
                    <label style={{ display: 'block', fontSize: '0.82rem', color: '#64748b', marginBottom: '4px' }}>Confirm New Password</label>
                    <input type="password" required style={{ width: '100%', padding: '10px 14px', borderRadius: '8px', border: '1px solid #cbd5e1', outline: 'none' }} placeholder="Confirm new password" />
                  </div>
                  <button type="submit" className="btn btn-primary" style={{ alignSelf: 'flex-start', marginTop: '4px', background: '#4f46e5', border: 'none' }}>
                    Update Password
                  </button>
                </form>
              </div>
              ...
              <div style={{ border: '1px solid #e2e8f0', borderRadius: '12px', padding: '16px' }}>
                <h4 style={{ fontSize: '1rem', color: '#1e293b', fontWeight: 600, margin: '0 0 16px 0', display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <FiBell color="#059669" /> Notifications & Preferences
                </h4>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div>
                      <div style={{ fontSize: '0.9rem', fontWeight: 500, color: '#334155' }}>Email Alerts</div>
                      <div style={{ fontSize: '0.78rem', color: '#64748b' }}>Receive updates on appointments and reports</div>
                    </div>
                    <label style={{ position: 'relative', display: 'inline-block', width: '44px', height: '24px' }}>
                      <input type="checkbox" defaultChecked style={{ opacity: 0, width: 0, height: 0 }} onChange={(e) => toast.success(`Email alerts ${e.target.checked ? 'enabled' : 'disabled'}`)} />
                      <span style={{ position: 'absolute', cursor: 'pointer', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: '#4f46e5', borderRadius: '24px', transition: '0.4s' }}>
                        <span style={{ position: 'absolute', content: '""', height: '18px', width: '18px', left: '22px', bottom: '3px', backgroundColor: 'white', borderRadius: '50%', transition: '0.4s' }}></span>
                      </span>
                    </label>
                  </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div>
                      <div style={{ fontSize: '0.9rem', fontWeight: 500, color: '#334155' }}>SMS Alerts</div>
                      <div style={{ fontSize: '0.78rem', color: '#64748b' }}>Get important SOS and emergency notifications</div>
                    </div>
                    <label style={{ position: 'relative', display: 'inline-block', width: '44px', height: '24px' }}>
                      <input type="checkbox" defaultChecked style={{ opacity: 0, width: 0, height: 0 }} onChange={(e) => toast.success(`SMS alerts ${e.target.checked ? 'enabled' : 'disabled'}`)} />
                      <span style={{ position: 'absolute', cursor: 'pointer', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: '#4f46e5', borderRadius: '24px', transition: '0.4s' }}>
                        <span style={{ position: 'absolute', content: '""', height: '18px', width: '18px', left: '22px', bottom: '3px', backgroundColor: 'white', borderRadius: '50%', transition: '0.4s' }}></span>
                      </span>
                    </label>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div>
                      <div style={{ fontSize: '0.9rem', fontWeight: 500, color: '#334155' }}>Browser Push Notifications</div>
                      <div style={{ fontSize: '0.78rem', color: '#64748b' }}>Allow MedAstraQ to send system messages</div>
                    </div>
                    <label style={{ position: 'relative', display: 'inline-block', width: '44px', height: '24px' }}>
                      <input type="checkbox" style={{ opacity: 0, width: 0, height: 0 }} onChange={(e) => toast.success(`Push notifications ${e.target.checked ? 'enabled' : 'disabled'}`)} />
                      <span style={{ position: 'absolute', cursor: 'pointer', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: '#cbd5e1', borderRadius: '24px', transition: '0.4s' }}>
                        <span style={{ position: 'absolute', content: '""', height: '18px', width: '18px', left: '4px', bottom: '3px', backgroundColor: 'white', borderRadius: '50%', transition: '0.4s' }}></span>
                      </span>
                    </label>
                  </div>
                </div>
